...
export const OMNIPOTENT_AUTHORITY: string[] = [
  ...
  // Claims added by the modules merged since the W034-era base (claimed by
  // the 39-a/b/d/f sweeps): rewards, workforce, briefings, api key
  // administration, and both marketplace sides. NOTE: 'marketplace:administer'
  // deliberately carries cross-tenant visibility on the platform pipeline
  // (review queue, administer-side reads) — that is a platform surface, not
  // a tenant-scope bypass; the vendor-side blindness probes in
  // platform-sweep.test.ts key on tenant-scoped operations.
  'rewards:administer',
  'workforce:decide',
