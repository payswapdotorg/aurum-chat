 ...
 * `llm_availability_events.seq` / `agent_runtime_availability_events.seq`
 * are global serials whose values are minted per ROW by the database (never
 * caller-supplied natural keys), so two tenants can never collide on them.
 * They carry no cross-tenant business meaning. Everything else must either
 * include tenant_id or be a surrogate uuid reference set.
 */
const TECHNICAL_UNIQUE_COLUMNS = new Set([
  'llm_executions.seq',
...
/**
 * Unique constraints that are deliberate GLOBAL credential anchors:
 * `api_keys.key_hash` is the sha-256 digest of a system-minted random key,
 * never a caller-supplied business natural key. Its global uniqueness IS the
 * authentication invariant (a presented key must resolve onto exactly ONE
 * tenant — W038's migration documents this contract), so namespacing it by
 * tenant_id would weaken authentication instead of hardening it. A digest
 * collision between tenants would be both practically impossible (sha-256 of
...
describe('W044 repository boundary — unique namespaces stay per-tenant', () => {
  it('includes tenant_id (or only globally-unique surrogate uuids) in every UNIQUE constraint', async () => {
    ...
    for (const constraint of constraints.rows) {
      ...
      if (columns.every((column) => GLOBAL_CREDENTIAL_ANCHORS.has(`${constraint.relname}.${column}`))) {
        continue; // documented global credential anchors (auth invariant)
      }
  ...
  it('includes tenant_id in every standalone UNIQUE index (including partial/expression indexes)', async () => {
    ...
    for (const index of indexes.rows) {
      ...
      // Column expression list is the balanced paren group that opens right
      // after "USING <method> (". Finding 3 (attempt 5): the previous greedy
      // `(.*)\)\s*(?:WHERE|$)` spanned INTO a partial index's WHERE predicate
      // and its nested parens, so an all-surrogate partial index like
      // api_webhook_deliveries (subscription_id, event_id) WHERE (...) was
      // mis-parsed as a namespace violation. Balanced-paren scanning keeps
      // the predicate out while still spanning nested expression parens
      // (e.g. COALESCE(install_key, '')).
      const openMatch = /USING\s+\w+\s*\(/.exec(index.indexdef);
      if (openMatch === null) {
        violations.push(`${index.tablename}.${index.indexname} (unparsable: ${index.indexdef})`);
        continue;
      }
      const openAt = openMatch.index + openMatch[0].length;
      // The opening paren is already consumed by openMatch, so the scan
      // starts INSIDE the group at depth 1 and closes at the matching ')'.
      let depth = 1;
      let closeAt = -1;
      for (let i = openAt; i < index.indexdef.length; i += 1) {
        const char = index.indexdef[i];
        if (char === '(') depth += 1;
        else if (char === ')') {
          depth -= 1;
          if (depth === 0) {
      ...
      const expressions = index.indexdef
        .slice(openAt, closeAt)
        .split(',')
        .map((part) => part.trim());
      ...
      violations.push(`${index.tablename}.${index.indexname} (${index.indexdef.slice(openAt, closeAt)})`);
    }
