...
describe('contract surface (the module exposes exactly its public operations)', () => {
  it('pins the export set — no manifest mutation, no run deletion, no un-deprecation', async () => {
    ...
    // and UI documents are replaceable, never deleted. The W027
    // additions are the builder's workflow operations and pure rule
    // set; there is deliberately no deleteBuild, no deleteArtifact and
    // no build-phase rewrite — builds and their custody are history,
    // and a failed build is evidence, never an edit.
