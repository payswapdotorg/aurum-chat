...
import { firstValue } from '../lib/context';
import {
  SESSION_COOKIE_NAME,
...
// ---------------------------------------------------------------------------
// Context resolution (W058 — the session replaced the dev seam)
// ---------------------------------------------------------------------------

describe('connections context resolution (the session-cookie carrier)', () => {
  it('the hub reads its scope from the session cookie only', () => {
    expect(
      sessionTokenFromCookieHeader(`other=1; ${SESSION_COOKIE_NAME}=hub-token`),
    ).toBe('hub-token');
    expect(sessionTokenFromCookieHeader('?tenant=globex')).toBeNull();
  });

  it('firstValue still powers the identity lookup parameters', () => {
    expect(firstValue(['whatsapp', 'slack'])).toBe('whatsapp');
    expect(firstValue('solo')).toBe('solo');
    expect(firstValue(undefined)).toBeNull();
  });

  it('the seam module is gone (no silent return of ?tenant=)', () => {
    for (const gone of [
